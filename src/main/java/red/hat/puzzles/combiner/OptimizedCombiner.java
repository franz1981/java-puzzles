...
public final class OptimizedCombiner extends Pad1Combiner implements Combiner {
    ...
    private ThreadLocal<Node> _myNode = ThreadLocal.withInitial(Node::new);

    private final int _limit;

    public OptimizedCombiner(int limit) {
        _limit = limit;
        _tail = new Node();
    }
    ...
    public void combine(Runnable action, Combiner.IdleStrategy idleStrategy) {
        ...
        Node n = curNode;
        Node nn;
        final int _limit = this._limit;
        for (int c = 0; c < _limit && (nn = n.lvNext()) != null; ++c, n = nn) {
            try {
                n.request.run();
            } finally {
                n.next = null;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(ClassLayout.parseClass(OptimizedCombiner.class).toPrintable());
    }
}
